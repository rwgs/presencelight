...
Phase 1 needs one account, so a single-tenant registration is sufficient. The later multi-account phase must revisit this if the two business accounts belong to different tenants: one registration would then need `-Audience MultiTenant` and consent in the second tenant, and unverified-publisher restrictions may apply there.
