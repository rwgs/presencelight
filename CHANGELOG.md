...
- Statuses set to Off now show as Off. The checkbox beside each status colour was bound to a MudBlazor parameter that no longer exists, so it always rendered unticked whatever was stored, while clicking it still wrote the value. A status could be switched off by a single click and leave no visible trace, which is what made in-a-call turn the light off with nothing in the interface to explain it. The control is shared by the Hue, LIFX, Wiz and Yeelight pages.
- The light no longer holds a stale colour when Microsoft Graph cannot be reached at all. The profile read fails before the freshness check and its failure skipped the rest of the polling cycle, so an application that started during an outage could show someone as available for as long as the outage lasted.
