...
Status: T1, T2, T4 and T5 complete; the desktop application builds, signs in, and drives the paired Hue light correctly through every presence state that was exercised. T6, freshness and failure indication, is the next task and now has its agreed indication. Phase order lives in [ROADMAP.md](ROADMAP.md), requirements in [SPEC.md](SPEC.md), and the current approach in [PLAN.md](PLAN.md).
...
| Recovery, latency and working-day trial | Latency measured 2026-09-09 for the application's own share: presence-to-bridge 0.04-0.43 s and a 5.24 s median poll interval, so under 6 s from the application seeing a change. Microsoft Graph's own propagation delay is not isolated and neither recovery nor the working-day trial has been run |
