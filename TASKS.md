...
Status: T1 and T2 complete; the desktop application builds, signs in and reads presence. Pairing the Hue Bridge (T4) is the next blocker. Phase order lives in [ROADMAP.md](ROADMAP.md), requirements in [SPEC.md](SPEC.md), and the current approach in [PLAN.md](PLAN.md).
...
- [ ] T4: Pair the bridge and control the selected light manually.
  - Progress 2026-09-08: discovery works. `Find Hue Bridge` located the bridge and saved its address, but pairing failed: the bridge returned `LinkButtonNotPressedException`, so no API key was created and light enumeration then failed with "Hue Api Key not configured". Two defects in that path are now repaired. [HueService.cs](src/PresenceLight.Core/Lights/HueServices/HueService.cs) made a single registration attempt, which fails whenever it races the physical button press; it now retries every 2 seconds for the bridge's 30-second link window, so the button can be pressed before or after confirming the dialog. [HueSetup.razor](src/PresenceLight.Razor/Components/Pages/HueSetup.razor) logged "Hue Bridge Registration Successful" before checking whether a key had actually been returned, and showed nothing while registration was in progress; it now logs success only when a key exists and shows a waiting message. The desktop build passes. Neither change has been exercised against the bridge, so pairing remains unverified and the change is not in [CHANGELOG.md](CHANGELOG.md) yet. Verified 2026-09-09 00:17 on the repaired build: pairing succeeded with no error, a bridge key was stored, the lights enumerated and a light was selected and saved (`id:/lights/2`). The two attempts at 00:12 and 00:13 that failed the same way as the previous evening were still on the old build, which was replaced at 00:14. What remains for this task is the colour check on the selected light. When the colour test runs, remember that working hours are enabled, so presence-driven updates only happen between 08:00 and 18:30 on weekdays; outside that window turn working hours off first or the light will not follow presence even once pairing succeeds. A third defect in the same path was found and repaired on 2026-09-09 while preparing this repair for upstream: `RegisterBridge` returns an empty key when the 30-second window closes without a press, and does so without throwing, so the catch in [HueSetup.razor](src/PresenceLight.Razor/Components/Pages/HueSetup.razor) never ran and the waiting message stayed on screen indefinitely, leaving a timed-out pairing indistinguishable from one still in progress. It now reports the expiry. That message is the only part of the repair not yet exercised, because the verified run paired successfully rather than timing out. The whole repair went upstream as [PR 990](https://github.com/isaacrlevin/presencelight/pull/990).
...
| Graph token recovery across restart and reauthentication | Not tested; T3 |
...
| Hue pairing and physical transitions | Pairing passed 2026-09-09 after the link-window repair: bridge key stored, lights enumerated, selection saved. Colour change on the selected light not yet checked |
