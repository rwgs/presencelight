...
A registration created this way is not publisher verified. Because PresenceLight requests
`Presence.Read`, which is beyond basic sign-in, an ordinary user in a tenant other than the one
holding the registration cannot consent for themselves where risk-based step-up consent applies. An
administrator of that tenant grants consent instead, which is what **Open consent page** does.
Publisher verification would lift the restriction, but it requires a verified Microsoft AI Cloud
Partner Program account for a legal business entity.
